...
fig_size = (16, 9)
color_list = ["r", "k", "y", "g", "c", "b", "m", "chocolate", "darkorange", "lime", "navy", "deeppink"]
word_sep = "_"
...
def kmeans_label_plot(input_df, num_cluster):
    kmeans_model = KMeans(n_clusters=num_cluster).fit(input_df)
    centroids = kmeans_model.cluster_centers_
    # print(f"{centroids=}")
    plt.scatter(input_df[long_str], input_df[lat_str], c=kmeans_model.labels_.astype(float), s=50, alpha=0.5)
...
def run_k_means(input_df, num_cluster, scope, fig_title):
    if (scope == Scope.US):
    ...
    for index in range(num_cluster):
        map_plotter.scatter(cluster_vals[str(index) + long_postfix], cluster_vals[str(index) + lat_postfix],
                            latlon=True, c=color_list[index + 1])
...
def main():
    ...
    num_us_cluster = 5
    num_world_cluster = 7
    df_us[kmlb_str] = kmeans_label_plot(df_us[log_lat_col_name], num_us_cluster)
    df_world[kmlb_str] = kmeans_label_plot(df_world[log_lat_col_name], num_world_cluster)
    ...
    for virus in viruses:
        ...
        print(f"Map of the US {virus} outbreak")
        run_k_means(df_us_virus, num_us_cluster, Scope.US, f"US {virus} outbreak")
        df_us_virus.to_csv(f'{data_path}\\{virus}_disease_outbreak_us.csv', index=False)

        df_world_virus = search_dataframe(df_world, virus)
        print(f"Map of the world {virus} outbreak")
        run_k_means(df_world_virus, num_world_cluster, Scope.world, f"world {virus} outbreak")
