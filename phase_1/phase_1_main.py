...
# 功能：从新闻标题中找国家名称
# 注意：传入的是已经unicode了的标题和已经unicode了的正式城市名，返回值也unicode了；该函数在调用match_city_country后调用
...
# 功能：根据标题匹配城市和初步匹配国家
# 注意：传入的是已经unicode了的标题，返回值也unicode了；该函数要先于match_country_from_headline函数调用
def match_city_country(headline):
    ...
    # 存在较多相同名字的us_county，但是没有关系，因为对应的国家都是United States
    matched_us_county = []
